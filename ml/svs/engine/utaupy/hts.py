...
def parse_phoneme_context(ctx_str: str) -> PhonemeContext:
    """
    例: "p@xx^xx-pau+k=a" のようなコンテキスト文字列を受け取り、
    PhonemeContext インスタンスを返す。
    """
    tokens = re.split(r"[@\^\-\+]", ctx_str)
    tokens = [t for t in tokens if t]  # 空文字を除去
    if len(tokens) != 5:
        raise ValueError(f"音素コンテキストが5つ見つかりませんでした: {tokens!r}")

    # 最後のトークン（next2）を、さらに '=' で分解する
    if "=" in tokens[4]:
        next2, next3 = tokens[4].split("=")
    else:
        next2 = tokens[4]
        next3 = ""  # 次々音素がなければ空文字列にしておく
...
class HTSFullLabel(UserList):
    ...
    def write(
        ...
        """
        ファイル出力する
        HACK: カスタムフラグはプロジェクトに組み込むために強引に拡張した。なのでwriteしないでdataclass(CustomNotes)を返す
        ...
        if custom:
            custom_notes = get_custom_notes(self)
            # writeという関数だが実際には書き込みせずdataclassを返す
            return custom_notes
        else:
            # ファイル出力
...
class Song(UserList):
    ...
    def write(
        ...
        mode="w",
        encoding="utf-8",
    ) -> Union[HTSFullLabel, _label.Label, CustomNotes]:
        """
        ファイル出力する。
        ...
        if custom and isinstance(ret, CustomNotes):
            return ret
        return full_label
...
def get_custom_notes(full_label: HTSFullLabel) -> CustomNotes:
    """
    ノートの音高情報を取得する
    """
    custom_notes: CustomNotes = CustomNotes(notes=[])
    new_label = deepcopy(full_label)

    for ol in new_label:
        pitch_name = ol.e[0:3][0]
        ol_str = str(ol)
        start_time = int(ol_str.split(" ")[0])
        end_time = int(ol_str.split(" ")[1])
        notenum = ABSPITCH_TO_NOTENUM_DICT[pitch_name]
        contexts = str(ol).split("/")
        phoneme_part = contexts[0].split(" ")[-1].split("_")[0]
        phoneme_context = parse_phoneme_context(phoneme_part)
        ...
        custom_notes.notes.append(custom_note)
