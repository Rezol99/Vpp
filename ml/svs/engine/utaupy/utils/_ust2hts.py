...
def ustobj2songobj(
    ust: up.ust.Ust, d_table: dict, key_of_the_note: int = None
) -> up.hts.Song:
    """
...
def ust2hts(
    ...
    # ファイル出力
    return hts_song.write(
        path_hts, strict_sinsy_style=strict_sinsy_style, as_mono=as_mono, custom=custom
    )
...
def main():
    ...
    for path_in in ust_files:
        try:
            ...
            raise Exception(
                f"Some exception was raised while processing {path_in}"
            ) from e
